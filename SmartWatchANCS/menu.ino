...
void playGame(bool &bjActive) {
  if (credits > 0) {
    ...
    const unsigned long buttonCheckInterval = 1000;
    //menu
    display.setCursor(0,13);
    display.print("<");
    display.setCursor(10,13);
    display.print("Big Small Game");
    display.setCursor(0,53);
    display.print("< Bigger");
    display.setCursor(50,53);
    display.print("Smaller >");
    
    for (int i = 0; i < 2; i++) {
      userCards[userNumCards++] = dealCard();
      computerCards[computerNumCards++] = dealCard();
    }

    int userScore, computerScore;

    //game
    userScore = calculateScore(userCards, userNumCards);
    computerScore = calculateScore(computerCards, computerNumCards);
    display.setCursor(10,23);
    display.print("Your cards: ");
    display.setCursor(10,33);
    for (int i = 0; i < userNumCards; i++) {
      display.print(userCards[i]);
      display.setCursor(25,33);
    }
    delay(1000);
    //back to menu button
    while (userChoice == 0 && bjActive){
      if (display.getButtons(TSButtonLowerLeft)){
        userChoice = 1;
      } else if (display.getButtons(TSButtonLowerRight)){
        userChoice = 2;
    ...
    //computer's results
    display.setCursor(10,23);
    display.print("Computer cards: ");
    display.setCursor(10,33);
    for (int i = 0; i < computerNumCards; i++) {
      display.print(computerCards[i]);
      display.setCursor(25,33);
    }

    //comparison of big/small
    ...
    //win/lose based on user selection
    display.setCursor(10,43);
    if (userChoice == 1 && bigWin == 1) {
      display.print("You win!!!");
      credits++;
    } else if (userChoice == 2 && smallWin == 1) {
      display.print("You win!!!");
      credits++;
    } else if (draw == 1) {
